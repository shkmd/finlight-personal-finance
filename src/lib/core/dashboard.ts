...
import { getBudgetVsActualCore } from "@/lib/actions/budget";
...
/** Shared with the web Server Action (src/lib/actions/dashboard.ts) and the mobile route handler — see src/lib/core/accounts.ts for the split rationale. */
export async function getDashboardSummaryCore(userId: string, target?: { year: number; month: number }) {
  ...
  const [
    ...
    getBudgetVsActualCore(userId, year, month),
